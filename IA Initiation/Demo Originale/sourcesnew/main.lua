...
function UpdateZombie(psZombie, psEntities)
  ...
  elseif psZombie.state == ZS.WALK then
    ...
    for i,sprite in ipairs(psEntities) do
      ...
      elseif sprite.type == "zombie" then
        if math.dist(psZombie.x, psZombie.y, sprite.x, sprite.y) < psZombie.range * 2
            and sprite.state == ZS.BITE then
          psZombie.state = ZS.ATTACK
          psZombie.target = sprite
        end
      end
  ...
  elseif psZombie.state == ZS.ATTACK then
    ...
    elseif psZombie.target.dead == true then
      psZombie.state = ZS.CHANGE
    elseif math.dist(psZombie.x, psZombie.y, psZombie.target.x, psZombie.target.y) > psZombie.range * 2
      and psZombie.target == "zombie" then
      psZombie.state = ZS.CHANGE
    elseif math.dist(psZombie.x, psZombie.y, psZombie.target.x, psZombie.target.y) > psZombie.range
      and psZombie.target == "human" then
      psZombie.state = ZS.CHANGE
    elseif psZombie.target.life <= 0 then
      psZombie.state = ZS.CHANGE
    elseif math.dist(psZombie.x, psZombie.y, psZombie.target.x, psZombie.target.y) < 5
      and psZombie.target.type == "human" then
      psZombie.vx = 0
      psZombie.vy = 0
      psZombie.state = ZS.BITE
    elseif math.dist(psZombie.x, psZombie.y, psZombie.target.x, psZombie.target.y) < 5
      and psZombie.target.type == "zombie" then
      psZombie.vx = 0
      psZombie.vy = 0
  ...
  elseif psZombie.state == ZS.BITE then
    ...
    if math.dist(psZombie.x, psZombie.y, psZombie.target.x, psZombie.target.y) > 5 then
      psZombie.state = ZS.ATTACK
    end
  -- ======================== --
  -- CHANGE
